...
class ReflectionOutput(BaseModel):
    ...
    current_company: str = Field(default="", description="Name of the current employer/company")
    current_role: str = Field(default="", description="Current job title or position")
    prior_companies: list[str] = Field(default_factory=list, description="List of previous companies worked at")
    education: str = Field(default="", description="Educational background and qualifications")
    skills: list[str] = Field(default_factory=list, description="Key technical and professional skills")
    notable_achievements: list[str] = Field(default_factory=list, description="Significant accomplishments or projects")
    
    # Completeness assessment
    is_complete: bool = Field(description="Whether the research gathered all essential information")
    missing_information: list[str] = Field(default_factory=list, description="List of missing information that should be searched")
    confidence_score: float = Field(default=0.0, description="Confidence score of the research completeness (0-1)")
    reasoning: str = Field(description="Reasoning for the completeness assessment and decision to continue or finish")
    suggested_queries: list[str] = Field(default_factory=list, description="Suggested search queries if more research is needed")
    
    # Decision
    decision: Literal["continue", "complete"] = Field(description="Decision to continue research or complete")
...
async def reflection(state: OverallState, config: RunnableConfig) -> dict[str, Any]:
    ...
    output_update = {
        "years_of_experience": reflection_result.years_of_experience if reflection_result.years_of_experience > 0 else None,
        "current_company": reflection_result.current_company if reflection_result.current_company else None,
        "current_role": reflection_result.current_role if reflection_result.current_role else None,
        "prior_companies": reflection_result.prior_companies,
        "education": reflection_result.education if reflection_result.education else None,
    ...
    if reflection_result.decision == "continue" and reflection_result.suggested_queries:
        output_update["search_queries"] = reflection_result.suggested_queries[:3]  # Limit to 3 queries
